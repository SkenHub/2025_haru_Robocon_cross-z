...
uint8_t send_data_enc[6];   // エンコーダ関連CAN送信データ
...
void can(void){
	...
	//sken_system.canTransmit(CAN_2, 0x360, send_data_enc, 6, 1);
...
int main(void) {
    ...
    /*limit[0].init(B15, INPUT);
    limit[1].init(B14, INPUT);
    limit[2].init(A11, INPUT);
    limit[3].init(A8, INPUT);
    limit[4].init(A7, INPUT);
    limit[5].init(A6, INPUT);
    limit[6].init(B9, INPUT);
    limit[7].init(B8, INPUT);*/
